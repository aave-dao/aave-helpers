...
library AaveV4EthereumSpokeHelpers {
  function getSpokes() internal pure returns (ISpoke[] memory) {
    ...
    spokes[10] = ISpoke(address(AaveV4EthereumSpokes.TREASURY_SPOKE));
    return spokes;
  }
...
library AaveV4EthereumTokenizationSpokeHelpers {
  function getTokenizationSpokes() internal pure returns (address[] memory) {
    ...
    spokes[22] = address(AaveV4EthereumTokenizationSpokes.PLUS_PT_sUSDE_7MAY2026_TOKENIZATION_SPOKE);
